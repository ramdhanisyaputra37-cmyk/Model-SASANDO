...
for k = 1:6
    subplot(2,3,k);
    n = t_snap(k);
    plot(x, u_eks(:,n), 'r-',  'LineWidth', 0.5); hold on;
    plot(x, u_imp(:,n), 'b--', 'LineWidth', 0.5);
    plot(x, u_df(:,n),  'g:',  'LineWidth', 0.5);
...
% ====== PLOT EVOLUSI WAKTU 3 TITIK ======
mid = round(Nx/2);       % x = 0.50l
qtr = round(Nx/4);       % x = 0.25l
tqr = round(3*Nx/4);     % x = 0.75l

figure(2);

subplot(1,3,1);
plot(t, u_eks(qtr,:), 'r-',  'LineWidth', 0.5); hold on;
plot(t, u_imp(qtr,:), 'b--', 'LineWidth', 0.5);
plot(t, u_df(qtr,:),  'g:',  'LineWidth', 0.5);
xlabel('t (s)'); ylabel('u');
title('x = 0.25l');
legend('Eksplisit','Implisit','Dufort-F.','Location','northeast');
grid on;
...
subplot(1,3,2);
plot(t, u_eks(mid,:), 'r-',  'LineWidth', 0.5); hold on;
plot(t, u_imp(mid,:), 'b--', 'LineWidth', 0.5);
plot(t, u_df(mid,:),  'g:',  'LineWidth', 0.5);
xlabel('t (s)'); ylabel('u');
title('x = 0.50l  (tengah)');
grid on;

subplot(1,3,3);
plot(t, u_eks(tqr,:), 'r-',  'LineWidth', 0.5); hold on;
plot(t, u_imp(tqr,:), 'b--', 'LineWidth', 0.5);
plot(t, u_df(tqr,:),  'g:',  'LineWidth', 0.5);
xlabel('t (s)'); ylabel('u');
title('x = 0.75l');
grid on;
